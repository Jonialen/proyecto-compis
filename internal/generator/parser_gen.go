...
func buildActionRows(table yapar.TableView) []parserActionRowData {
	...
	for _, state := range states {
		...
		for _, symbol := range terminals {
			action, ok := actionFromView(table, state, symbol)
			if !ok {
				continue
			}
...
func (v parserTableViewAdapter) ActionAt(state int, symbol string) (yapar.ActionKind, int, bool) {
...
func actionFromView(table yapar.TableView, state int, symbol string) (yapar.Action, bool) {
	kind, value, ok := table.ActionAt(state, symbol)
	if !ok {
		return yapar.Action{}, false
	}
	action := yapar.Action{Kind: kind}
	switch kind {
	case yapar.ActionShift:
		action.TargetState = value
	case yapar.ActionReduce:
		action.ProductionID = value
	}
	return action, true
}

func actionValue(action yapar.Action) int {
	switch action.Kind {
	case yapar.ActionShift:
