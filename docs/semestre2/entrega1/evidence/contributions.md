...
```text
50  jonialen <jonathanalej09@gmail.com>
...
The command reports 77 commits across four author signatures. The first two signatures share the same email and differ only by capitalization, so they are aliases for one observable contributor identity (67 commits combined). The GitHub noreply signature contains the same `Jonialen` account handle; it is documented as another Jonathan/Jonialen alias (1 commit), rather than as a third person. `DijanU <pad23663@uvg.edu.gt>` is the second observable contributor identity (9 commits).
