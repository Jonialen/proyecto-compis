...
- [x] 2.4 `compiscript-classes-exceptions` (2.3): class scopes, members, constructor arguments, class-only `this`, inheritance/lookup/cycles, inherited-name ban, non-inherited constructors; catch-only exception binding, both blocks analyzed, no `throw` in `semantic/`. **Status:** completed under ordinary Git policy as one 400-line direct unit; no SDD PASS/archive claimed. **Test:** `go test ./internal/compiscript/semantic -run 'Test(Classes|Catch)' -count=1 && go test ./... -count=1 && go build ./...`. **Harness:** N/A—library. **Rollback:** `internal/compiscript/semantic/{semantic.go,classes_exceptions_test.go,testdata/{classes_valid.cps,classes_invalid.cps,catch_valid.cps,catch_invalid.cps}}` and this status hunk.
