...
- [x] 2.3 `compiscript-collections` (2.2): homogeneous recursive lists/matrices, order-independent contextual empties, integer/indexed assignment checks, provable literal/simple-integer/named-constant bounds, conservative alias invalidation, and ErrorType writes that preserve prior shape facts. **Status:** completed under ordinary Git policy as a 373-line final snapshot (364-line candidate + 9-line net correction); no SDD PASS/archive claimed. **Test:** `go test ./internal/compiscript/semantic -run 'TestCollections' -count=1 && go test ./internal/compiscript -run 'TestAnalyze' -count=1 && go test ./... -count=1 && go build ./...`. **Harness:** N/A—library. **Rollback:** `internal/compiscript/semantic/{semantic.go,collections_test.go,testdata/{collections_valid.cps,collections_invalid.cps}}` and this status hunk.
- [x] 2.4 `compiscript-classes-exceptions` (2.3): class scopes, members, constructor arguments, lexical `this`, inheritance/lookup/cycles, inherited-name ban, non-inherited constructors; catch-only exception binding, both blocks analyzed, no `throw` in `semantic/`. **Status:** completed under ordinary Git policy as the retained 400-line direct unit plus an independent 123-line correction for assignable property mapping, lexical nested classes, exact catch-name spans, and nested-function `this`; no SDD PASS/archive claimed. **Test:** focused AST/frontend/classes/catch and semantic/facade regressions, `go test ./... -count=1`, and `go build ./...`. **Harness:** N/A—library. **Rollback:** the original seven-file unit remains independent; correction is limited to `internal/compiscript/{ast/{nodes.go,ast_test.go},frontend/{mapper_statement.go,mapper_statement_test.go},semantic/{semantic.go,classes_exceptions_test.go,testdata/{classes_valid.cps,classes_invalid.cps}}}` and this status hunk.

## Phase 3: Consumers

- [x] 3.1 `compiscript-cli` (2.4): `.cps` input and deterministic facade-equivalent JSON in `cmd/compiscript/`; documented valid/invalid runs. **Status:** completed under ordinary Git policy as a 245-line direct unit, including a 22-line UTF-8 hardening correction; diagnostics remain successful JSON results, usage/extension errors exit 2, and invalid UTF-8 or I/O errors exit 1 with clean stdout/stderr separation. **Test:** subprocess facade-equivalence/error tests, `go test ./cmd/compiscript -count=1`, `go test ./... -count=1`, dependency guard, check-only formatting, and `go build ./...`. **Harness:** valid and invalid `go run ./cmd/compiscript testdata/compiscript/{valid,invalid}/types.cps` runs exit 0 with deterministic JSON. **Rollback:** `cmd/compiscript/`, `testdata/compiscript/`, CLI evidence, and this status hunk.
