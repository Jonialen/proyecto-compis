...
- [x] 2.1 `compiscript-names-types` (1.2): nested/global resolution, shadowing, same-scope variable/function/parameter duplicates, unresolved names, function/block scopes; numeric promotion/float division, string `+`, arithmetic/logical/comparison/assignment rules, null, constants, invalid function operands, `ErrorType` suppression, exhaustive existing-node traversal, and deterministic visual-AST facade normalization. **Status:** completed under ordinary Git policy as two bounded direct units (398-line core + 387-line traversal/report); no SDD PASS/archive claimed. **Test:** `go test ./internal/compiscript ./internal/compiscript/semantic -run 'Test(Analyze|Names|Types)' -count=1 && go test ./... && go build ./...`. **Harness:** N/A—library. **Rollback:** `internal/compiscript/{analyze.go,view.go,analyze_test.go}`, `internal/compiscript/semantic/`, and the 2.1 status hunk.
